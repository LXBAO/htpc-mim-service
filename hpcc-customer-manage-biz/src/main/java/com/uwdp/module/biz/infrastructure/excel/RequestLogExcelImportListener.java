 ...
 */
@Slf4j
public class RequestLogExcelImportListener extends BaseExcelImportListener<RequestLogExcelImport> implements ReadListener<RequestLogExcelImport> {
    ...
    public RequestLogExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(RequestLogExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + RequestLogExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(RequestLogExcelImport.class);
    }

    @Override
    public void invoke(RequestLogExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, RequestLogExcelImport.class, METHOD_MAP)) {
