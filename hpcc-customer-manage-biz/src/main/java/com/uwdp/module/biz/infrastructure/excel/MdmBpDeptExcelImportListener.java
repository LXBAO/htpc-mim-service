...
/**
 * <p>
 * 主数据-业务编码所对应部门 数据导入监听器
 * </p>
 *
 */
@Slf4j
public class MdmBpDeptExcelImportListener extends BaseExcelImportListener<MdmBpDeptExcelImport> implements ReadListener<MdmBpDeptExcelImport> {
    ...
    public MdmBpDeptExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(MdmBpDeptExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + MdmBpDeptExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(MdmBpDeptExcelImport.class);
    }

    @Override
    public void invoke(MdmBpDeptExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, MdmBpDeptExcelImport.class, METHOD_MAP)) {
