 ...
 */
@Slf4j
public class MdmPersonExcelImportListener extends BaseExcelImportListener<MdmPersonExcelImport> implements ReadListener<MdmPersonExcelImport> {
    ...
    public MdmPersonExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(MdmPersonExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + MdmPersonExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(MdmPersonExcelImport.class);
    }

    @Override
    public void invoke(MdmPersonExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, MdmPersonExcelImport.class, METHOD_MAP)) {
