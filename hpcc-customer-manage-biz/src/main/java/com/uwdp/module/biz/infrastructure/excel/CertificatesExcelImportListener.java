 ...
 */
@Slf4j
public class CertificatesExcelImportListener extends BaseExcelImportListener<CertificatesExcelImport> implements ReadListener<CertificatesExcelImport> {
    ...
    public CertificatesExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(CertificatesExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + CertificatesExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(CertificatesExcelImport.class);
    }

    @Override
    public void invoke(CertificatesExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, CertificatesExcelImport.class, METHOD_MAP)) {
