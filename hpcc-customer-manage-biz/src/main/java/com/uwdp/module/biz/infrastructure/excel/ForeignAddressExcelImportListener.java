 ...
 */
@Slf4j
public class ForeignAddressExcelImportListener extends BaseExcelImportListener<ForeignAddressExcelImport> implements ReadListener<ForeignAddressExcelImport> {
    ...
    public ForeignAddressExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(ForeignAddressExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + ForeignAddressExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(ForeignAddressExcelImport.class);
    }

    @Override
    public void invoke(ForeignAddressExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, ForeignAddressExcelImport.class, METHOD_MAP)) {
