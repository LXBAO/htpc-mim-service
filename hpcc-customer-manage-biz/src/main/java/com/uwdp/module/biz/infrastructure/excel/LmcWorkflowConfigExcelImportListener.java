 ...
 */
@Slf4j
public class LmcWorkflowConfigExcelImportListener extends BaseExcelImportListener<LmcWorkflowConfigExcelImport> implements ReadListener<LmcWorkflowConfigExcelImport> {
    ...
    public LmcWorkflowConfigExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(LmcWorkflowConfigExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + LmcWorkflowConfigExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(LmcWorkflowConfigExcelImport.class);
    }

    @Override
    public void invoke(LmcWorkflowConfigExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, LmcWorkflowConfigExcelImport.class, METHOD_MAP)) {
