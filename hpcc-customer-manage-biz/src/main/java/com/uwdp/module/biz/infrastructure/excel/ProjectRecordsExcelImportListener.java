 ...
 */
@Slf4j
public class ProjectRecordsExcelImportListener extends BaseExcelImportListener<ProjectRecordsExcelImport> implements ReadListener<ProjectRecordsExcelImport> {
    ...
    public ProjectRecordsExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(ProjectRecordsExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + ProjectRecordsExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(ProjectRecordsExcelImport.class);
    }

    @Override
    public void invoke(ProjectRecordsExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, ProjectRecordsExcelImport.class, METHOD_MAP)) {
