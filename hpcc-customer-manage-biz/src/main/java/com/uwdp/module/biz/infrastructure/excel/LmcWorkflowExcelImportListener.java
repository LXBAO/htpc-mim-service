 ...
 */
@Slf4j
public class LmcWorkflowExcelImportListener extends BaseExcelImportListener<LmcWorkflowExcelImport> implements ReadListener<LmcWorkflowExcelImport> {
    ...
    public LmcWorkflowExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(LmcWorkflowExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + LmcWorkflowExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(LmcWorkflowExcelImport.class);
    }

    @Override
    public void invoke(LmcWorkflowExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, LmcWorkflowExcelImport.class, METHOD_MAP)) {
