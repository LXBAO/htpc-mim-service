 ...
 */
@Slf4j
public class PerformanceLedgerExcelImportListener extends BaseExcelImportListener<PerformanceLedgerExcelImport> implements ReadListener<PerformanceLedgerExcelImport> {
    ...
    public PerformanceLedgerExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(PerformanceLedgerExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + PerformanceLedgerExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(PerformanceLedgerExcelImport.class);
    }

    @Override
    public void invoke(PerformanceLedgerExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, PerformanceLedgerExcelImport.class, METHOD_MAP)) {
