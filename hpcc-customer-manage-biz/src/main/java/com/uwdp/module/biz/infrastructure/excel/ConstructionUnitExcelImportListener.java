 ...
 */
@Slf4j
public class ConstructionUnitExcelImportListener extends BaseExcelImportListener<ConstructionUnitExcelImport> implements ReadListener<ConstructionUnitExcelImport> {
    ...
    public ConstructionUnitExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(ConstructionUnitExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + ConstructionUnitExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(ConstructionUnitExcelImport.class);
    }

    @Override
    public void invoke(ConstructionUnitExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, ConstructionUnitExcelImport.class, METHOD_MAP)) {
