 ...
 */
@Slf4j
public class SectionExcelImportListener extends BaseExcelImportListener<SectionExcelImport> implements ReadListener<SectionExcelImport> {
    ...
    public SectionExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(SectionExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + SectionExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(SectionExcelImport.class);
    }

    @Override
    public void invoke(SectionExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, SectionExcelImport.class, METHOD_MAP)) {
