 ...
 */
@Slf4j
public class ProjectImplementExcelImportListener extends BaseExcelImportListener<ProjectImplementExcelImport> implements ReadListener<ProjectImplementExcelImport> {
    ...
    public ProjectImplementExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(ProjectImplementExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + ProjectImplementExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(ProjectImplementExcelImport.class);
    }

    @Override
    public void invoke(ProjectImplementExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, ProjectImplementExcelImport.class, METHOD_MAP)) {
