 ...
 */
@Slf4j
public class ProjectEnableExcelImportListener extends BaseExcelImportListener<ProjectEnableExcelImport> implements ReadListener<ProjectEnableExcelImport> {
    ...
    public ProjectEnableExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(ProjectEnableExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + ProjectEnableExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(ProjectEnableExcelImport.class);
    }

    @Override
    public void invoke(ProjectEnableExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, ProjectEnableExcelImport.class, METHOD_MAP)) {
