 ...
 */
@Slf4j
public class TimedTaskExcelImportListener extends BaseExcelImportListener<TimedTaskExcelImport> implements ReadListener<TimedTaskExcelImport> {
    ...
    public TimedTaskExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(TimedTaskExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + TimedTaskExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(TimedTaskExcelImport.class);
    }

    @Override
    public void invoke(TimedTaskExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, TimedTaskExcelImport.class, METHOD_MAP)) {
