 ...
 */
@Slf4j
public class ProjectBiddingExcelImportListener extends BaseExcelImportListener<ProjectBiddingExcelImport> implements ReadListener<ProjectBiddingExcelImport> {
    ...
    public ProjectBiddingExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(ProjectBiddingExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + ProjectBiddingExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(ProjectBiddingExcelImport.class);
    }

    @Override
    public void invoke(ProjectBiddingExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, ProjectBiddingExcelImport.class, METHOD_MAP)) {
