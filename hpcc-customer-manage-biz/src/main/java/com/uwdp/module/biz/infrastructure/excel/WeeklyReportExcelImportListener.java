 ...
 */
@Slf4j
public class WeeklyReportExcelImportListener extends BaseExcelImportListener<WeeklyReportExcelImport> implements ReadListener<WeeklyReportExcelImport> {
    ...
    public WeeklyReportExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(WeeklyReportExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + WeeklyReportExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(WeeklyReportExcelImport.class);
    }

    @Override
    public void invoke(WeeklyReportExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, WeeklyReportExcelImport.class, METHOD_MAP)) {
