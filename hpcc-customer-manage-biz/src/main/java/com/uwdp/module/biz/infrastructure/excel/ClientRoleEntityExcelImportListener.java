 ...
 */
@Slf4j
public class ClientRoleEntityExcelImportListener extends BaseExcelImportListener<ClientRoleEntityExcelImport> implements ReadListener<ClientRoleEntityExcelImport> {
    ...
    public ClientRoleEntityExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(ClientRoleEntityExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + ClientRoleEntityExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(ClientRoleEntityExcelImport.class);
    }

    @Override
    public void invoke(ClientRoleEntityExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, ClientRoleEntityExcelImport.class, METHOD_MAP)) {
