 ...
 */
@Slf4j
public class CALedgerExcelImportListener extends BaseExcelImportListener<CALedgerExcelImport> implements ReadListener<CALedgerExcelImport> {
    ...
    public CALedgerExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(CALedgerExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + CALedgerExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(CALedgerExcelImport.class);
    }

    @Override
    public void invoke(CALedgerExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, CALedgerExcelImport.class, METHOD_MAP)) {
