 ...
 */
@Slf4j
public class LinkmanExcelImportListener extends BaseExcelImportListener<LinkmanExcelImport> implements ReadListener<LinkmanExcelImport> {
    ...
    public LinkmanExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(LinkmanExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + LinkmanExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(LinkmanExcelImport.class);
    }

    @Override
    public void invoke(LinkmanExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, LinkmanExcelImport.class, METHOD_MAP)) {
