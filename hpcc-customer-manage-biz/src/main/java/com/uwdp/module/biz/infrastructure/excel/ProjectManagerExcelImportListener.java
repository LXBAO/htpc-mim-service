 ...
 */
@Slf4j
public class ProjectManagerExcelImportListener extends BaseExcelImportListener<ProjectManagerExcelImport> implements ReadListener<ProjectManagerExcelImport> {
    ...
    public ProjectManagerExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(ProjectManagerExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + ProjectManagerExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(ProjectManagerExcelImport.class);
    }

    @Override
    public void invoke(ProjectManagerExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, ProjectManagerExcelImport.class, METHOD_MAP)) {
