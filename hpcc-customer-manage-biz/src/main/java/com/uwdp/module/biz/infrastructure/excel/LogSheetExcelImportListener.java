 ...
 */
@Slf4j
public class LogSheetExcelImportListener extends BaseExcelImportListener<LogSheetExcelImport> implements ReadListener<LogSheetExcelImport> {
    ...
    public LogSheetExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(LogSheetExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + LogSheetExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(LogSheetExcelImport.class);
    }

    @Override
    public void invoke(LogSheetExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, LogSheetExcelImport.class, METHOD_MAP)) {
