 ...
 */
@Slf4j
public class DBConfigExcelImportListener extends BaseExcelImportListener<DBConfigExcelImport> implements ReadListener<DBConfigExcelImport> {
    ...
    public DBConfigExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(DBConfigExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + DBConfigExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(DBConfigExcelImport.class);
    }

    @Override
    public void invoke(DBConfigExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, DBConfigExcelImport.class, METHOD_MAP)) {
