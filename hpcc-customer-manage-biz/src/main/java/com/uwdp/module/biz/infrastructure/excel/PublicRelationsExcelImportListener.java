 ...
 */
@Slf4j
public class PublicRelationsExcelImportListener extends BaseExcelImportListener<PublicRelationsExcelImport> implements ReadListener<PublicRelationsExcelImport> {
    ...
    public PublicRelationsExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(PublicRelationsExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + PublicRelationsExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(PublicRelationsExcelImport.class);
    }

    @Override
    public void invoke(PublicRelationsExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, PublicRelationsExcelImport.class, METHOD_MAP)) {
