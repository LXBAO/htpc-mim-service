 ...
 */
@Slf4j
public class MarketDmpTagExcelImportListener extends BaseExcelImportListener<MarketDmpTagExcelImport> implements ReadListener<MarketDmpTagExcelImport> {
    ...
    public MarketDmpTagExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(MarketDmpTagExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + MarketDmpTagExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(MarketDmpTagExcelImport.class);
    }

    @Override
    public void invoke(MarketDmpTagExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, MarketDmpTagExcelImport.class, METHOD_MAP)) {
