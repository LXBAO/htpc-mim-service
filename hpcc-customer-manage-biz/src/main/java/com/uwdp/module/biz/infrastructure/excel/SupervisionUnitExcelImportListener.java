 ...
 */
@Slf4j
public class SupervisionUnitExcelImportListener extends BaseExcelImportListener<SupervisionUnitExcelImport> implements ReadListener<SupervisionUnitExcelImport> {
    ...
    public SupervisionUnitExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(SupervisionUnitExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + SupervisionUnitExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(SupervisionUnitExcelImport.class);
    }

    @Override
    public void invoke(SupervisionUnitExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, SupervisionUnitExcelImport.class, METHOD_MAP)) {
