 ...
 */
@Slf4j
public class DataExcelImportListener extends BaseExcelImportListener<DataExcelImport> implements ReadListener<DataExcelImport> {
    ...
    public DataExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(DataExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + DataExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(DataExcelImport.class);
    }
    ...
    public void invoke(DataExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, DataExcelImport.class, METHOD_MAP)) {
