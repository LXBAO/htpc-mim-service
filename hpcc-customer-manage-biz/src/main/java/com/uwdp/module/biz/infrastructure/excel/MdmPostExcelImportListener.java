 ...
 */
@Slf4j
public class MdmPostExcelImportListener extends BaseExcelImportListener<MdmPostExcelImport> implements ReadListener<MdmPostExcelImport> {
    ...
    public MdmPostExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(MdmPostExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + MdmPostExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(MdmPostExcelImport.class);
    }

    @Override
    public void invoke(MdmPostExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, MdmPostExcelImport.class, METHOD_MAP)) {
