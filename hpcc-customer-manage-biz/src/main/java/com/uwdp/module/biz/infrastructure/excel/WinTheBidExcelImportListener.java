 ...
 */
@Slf4j
public class WinTheBidExcelImportListener extends BaseExcelImportListener<WinTheBidExcelImport> implements ReadListener<WinTheBidExcelImport> {
    ...
    public WinTheBidExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(WinTheBidExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + WinTheBidExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(WinTheBidExcelImport.class);
    }

    @Override
    public void invoke(WinTheBidExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, WinTheBidExcelImport.class, METHOD_MAP)) {
