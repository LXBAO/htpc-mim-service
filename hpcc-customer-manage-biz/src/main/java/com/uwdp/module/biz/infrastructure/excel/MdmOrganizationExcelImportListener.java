 ...
 */
@Slf4j
public class MdmOrganizationExcelImportListener extends BaseExcelImportListener<MdmOrganizationExcelImport> implements ReadListener<MdmOrganizationExcelImport> {
    ...
    public MdmOrganizationExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(MdmOrganizationExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + MdmOrganizationExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(MdmOrganizationExcelImport.class);
    }

    @Override
    public void invoke(MdmOrganizationExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, MdmOrganizationExcelImport.class, METHOD_MAP)) {
