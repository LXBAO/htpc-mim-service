 ...
 */
@Slf4j
public class VisitPlanExcelImportListener extends BaseExcelImportListener<VisitPlanExcelImport> implements ReadListener<VisitPlanExcelImport> {
    ...
    public VisitPlanExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(VisitPlanExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + VisitPlanExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(VisitPlanExcelImport.class);
    }

    @Override
    public void invoke(VisitPlanExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, VisitPlanExcelImport.class, METHOD_MAP)) {
