 ...
 */
@Slf4j
public class ProjectSigningExcelImportListener extends BaseExcelImportListener<ProjectSigningExcelImport> implements ReadListener<ProjectSigningExcelImport> {
    ...
    public ProjectSigningExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(ProjectSigningExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + ProjectSigningExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(ProjectSigningExcelImport.class);
    }

    @Override
    public void invoke(ProjectSigningExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, ProjectSigningExcelImport.class, METHOD_MAP)) {
