 ...
 */
@Slf4j
public class ContractPartyExcelImportListener extends BaseExcelImportListener<ContractPartyExcelImport> implements ReadListener<ContractPartyExcelImport> {
    ...
    public ContractPartyExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(ContractPartyExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + ContractPartyExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(ContractPartyExcelImport.class);
    }

    @Override
    public void invoke(ContractPartyExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, ContractPartyExcelImport.class, METHOD_MAP)) {
