 ...
 */
@Slf4j
public class AttachmentExcelImportListener extends BaseExcelImportListener<AttachmentExcelImport> implements ReadListener<AttachmentExcelImport> {
    ...
    public AttachmentExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(AttachmentExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + AttachmentExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(AttachmentExcelImport.class);
    }

    @Override
    public void invoke(AttachmentExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, AttachmentExcelImport.class, METHOD_MAP)) {
