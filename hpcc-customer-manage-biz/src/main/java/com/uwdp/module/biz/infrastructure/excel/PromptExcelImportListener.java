 ...
 */
@Slf4j
public class PromptExcelImportListener extends BaseExcelImportListener<PromptExcelImport> implements ReadListener<PromptExcelImport> {
    ...
    public PromptExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(PromptExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + PromptExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(PromptExcelImport.class);
    }

    @Override
    public void invoke(PromptExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, PromptExcelImport.class, METHOD_MAP)) {
