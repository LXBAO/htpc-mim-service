 ...
 */
@Slf4j
public class UpdateViewExcelImportListener extends BaseExcelImportListener<UpdateViewExcelImport> implements ReadListener<UpdateViewExcelImport> {
    ...
    public UpdateViewExcelImportListener() {
        if (!IS_INIT.get()) {
            try {
                BeanInfo beanInfo = Introspector.getBeanInfo(UpdateViewExcelImport.class);
                for (PropertyDescriptor propertyDescriptor : beanInfo.getPropertyDescriptors()) {
                    METHOD_MAP.put(propertyDescriptor.getReadMethod().getName(), propertyDescriptor.getName());
                }
                IS_INIT.set(Boolean.TRUE);
            } catch (Exception e) {
                throw new BizException("导入类: " + UpdateViewExcelImport.class.getName() + ", 解析错误");
            }
        }
        super.initAnnotationInfoList(UpdateViewExcelImport.class);
    }

    @Override
    public void invoke(UpdateViewExcelImport data, AnalysisContext context) {
        ...
        // 跳过错误的数据
        if (super.parseObjectField(data, UpdateViewExcelImport.class, METHOD_MAP)) {
