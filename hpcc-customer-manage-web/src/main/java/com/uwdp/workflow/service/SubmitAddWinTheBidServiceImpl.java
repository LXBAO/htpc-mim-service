...
import com.uwdp.workflow.service.iservice.IWinTheBidService;
...
public  class SubmitAddWinTheBidServiceImpl implements IWinTheBidService {

    @Value("${hpcc-mim-server.app-id}")
    public String appId; /*应用ID*/

    private static final String flowCode = BizCode.ADD_WIN_THE_BID.getValue(); /*流程编码*/

    public static final String[] FD_INPUT_TYPE = {"未使用","投标状态","在建状态"}; /*项目投标人员参与状态*/
    ...
    @Override
    public Response add(WinTheBidAddCmd addCmd) {
        ...
        // 项目更改为中标状态后更新对应的项目投标参与人员为在建状态
        String projectRecordId = addCmd.getItemNumber();
        this.updateFdInputType(projectRecordId,2);
        ...
        dto.setTitle("【市场管理】【"+addCmd.getProjectName()+"】项目中标审批单");
...
//            lmcWorkflowDo.setId();
        ...
        lmcWorkflowDo.setTitle("项目中标新增-"+addCmd.getProjectName());
    ...
    public Response reStartProcess(WinTheBidAddCmd addCmd) {
        ...
        LmcWorkflowDo workflowDo = Optional.ofNullable(
                ...
                () -> new BizRuntimeException(
                        "根据新增项目中标的Id:{" + addCmd.getId() + "}找不到对应流程实例",new Exception())
        );
        WinTheBidDo winTheBidDo = winTheBidAssembler.toDO(addCmd);
        winTheBidDo.setUpdatedTime(LocalDateTime.now());
        boolean bool = winTheBidRepository.updateById(winTheBidDo);
        if (!bool)log.error("重新发起流程:更新新增项目中标时失败!项目中标:{}",winTheBidDo);
        Map<String, Object> variables = BeanUtil.beanToMap(addCmd);
...
//        dto.setAppId(appId);
        ...
        log.info("==项目中标新增流程驳回后重新发起 响应:{}",response);
...
//        String fdId = projectRecordsService.addThenReturnId(addCmd);
        ...
        winTheBidDo.setUpdatedTime(LocalDateTime.now());
        boolean bool = winTheBidRepository.updateById(winTheBidDo);
        if (!bool)log.error("发起流程:更新新增项目中标时失败!项目中标:{}",winTheBidDo);
        Long fdId=winTheBidDo.getId();
...
//        timelineAddCmd.setId(0L);
    ...
    public void handlePass(ProcessCall processCall) {
        ...
        projectRecordsQuery.setId(Long.valueOf((String) processCall.getFormData().get("itemNumber")));
        projectRecordsQuery.setProjectPhase("4");
    ...
    public void handleBack(ProcessCall processCall) {
        ...
        String projectRecordId = (String) formData.get("itemNumber");
        this.updateFdInputType(projectRecordId,1);
        // 更新业务关联的工作流状态
...
//        timelineAddCmd.setWorkflowId(workflowAddCmd.getId().toString());
    ...
    /**
     * 项目更改为中标状态后更新对应的项目投标参与人员为在建状态
     * projectRecordId项目id，status项目经理状态
      */
    public void updateFdInputType(String projectRecordId,int status){
        log.info(">>>>>>>>项目中标流程审批通过 updateFdInputType...");
//        Map<String, Object> formData = processCall.getFormData();
//        String projectRecordId = (String) formData.get("itemNumber");
        log.info(">>>>>>>>中标的项目Id:{}",projectRecordId);

        List<LinkmanDo> linkmanDos = linkmanRepository.list(new QueryWrapper<LinkmanDo>()
                .eq("BIDDINGID", projectRecordId));

        if (null == linkmanDos || linkmanDos.size() == 0){
            log.info(">>>>>>>>根据中标的项目Id:{} , 没有查到相关的LinkMan",projectRecordId);
            return;
        }

        Object[] fdJobNumberS = linkmanDos.stream()
                .map(LinkmanDo::getNameId)
                .toArray();

        log.info(">>>>>>>>所有人员工号：{}",Arrays.toString(fdJobNumberS));

        UpdateWrapper<ProjectManagerLedgerDo> updateWrapper = new UpdateWrapper<>();
        updateWrapper.set("FDINPUTTYPE", FD_INPUT_TYPE[status]);
        updateWrapper.in("FDJOBNUMBER", fdJobNumberS);
        boolean update = projectManagerLedgerRepository.update(updateWrapper);
        if(update)log.info(">>>>>>>>更新项目经理表成功");
    }
