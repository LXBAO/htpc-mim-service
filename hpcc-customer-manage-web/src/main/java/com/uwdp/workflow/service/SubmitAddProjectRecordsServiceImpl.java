...
import com.uwdp.workflow.service.iservice.IProjectRecordsService;
...
public class SubmitAddProjectRecordsServiceImpl implements IProjectRecordsService {


    @Value("${hpcc-mim-server.app-id}")
    public String appId; /*应用ID*/

    private static final String flowCode = BizCode.ADD_PROJECT_RECORDS.getValue(); /*流程编码*/

    private final com.uwdp.module.api.service.IProjectRecordsService projectRecordsService;
    ...
    private final ILmcWorkflowVariableService iLmcWorkflowVariableService;
    ...
    @Override
    public Response add(ProjectRecordsAddCmd addCmd) {
        ...
        Long id = projectRecordsService.addThenReturnId(addCmd);
        ProjectRecordsDto projectRecordsDto = projectRecordsService.get(id);

        Map<String, Object> variables = BeanUtil.beanToMap(addCmd);

        variables.put("projectNumber", projectRecordsDto.getProjectNumber());
        ...
        dto.setTitle("【市场管理】【" + addCmd.getProjectName() + "】项目登记审批单");
//        dto.setAppId(appId);    // 应用id
        ...
        log.info(addCmd.getApproveUser().toString(), "这是审批人");
...
//            lmcWorkflowDo.setId();
        ...
        lmcWorkflowDo.setTitle("项目登记新增-" + addCmd.getFdName());
...
//        System.out.println(lmcWorkflowDo.getId());
        ...
        LmcWorkflowVariableAddCmd variableAddCmd = new LmcWorkflowVariableAddCmd();
        variableAddCmd.setMainId(id);
        variableAddCmd.setWorkflowId(lmcWorkflowDo.getId());
        variableAddCmd.setFormData(JSON.toJSONString(addCmd));
        variableAddCmd.setProcessData(JSON.toJSONString(dto));
        variableAddCmd.setMemo("首次发起");
        iLmcWorkflowVariableService.add(variableAddCmd);
...
//        timelineAddCmd.setId(0L);
    ...
    @Override
    public Response reStartProcess(ProjectRecordsAddCmd addCmd) {
        Boolean b = FunctionalUtil.verifyAndParseApproveUser(addCmd.getApproveUser());
        if (!b) {
            return Response.buildFailure("505","请指定审批人！");
        }
        LmcWorkflowDo workflowDo = Optional.ofNullable(
                ...
                () -> new BizRuntimeException(
                        "根据新增项目登记的Id:{" + addCmd.getId() + "}找不到对应流程实例", new Exception())
        );
        ProjectRecordsDo projectRecordsDo = projectRecordsAssembler.toDO(addCmd);
        projectRecordsDo.setUpdatedTime(LocalDateTime.now());
        boolean bool = projectRecordsRepository.updateById(projectRecordsDo);
        if (!bool) log.error("重新发起流程:更新新增项目登记时失败!项目登记:{}", projectRecordsDo);
        Map<String, Object> variables = BeanUtil.beanToMap(addCmd);
...
//        dto.setBusinessPositions();     // 业务岗
        ...
        log.info("==项目登记新增流程驳回后重新发起 响应:{}", response);
        ...
        LmcWorkflowVariableAddCmd variableAddCmd = new LmcWorkflowVariableAddCmd();
        variableAddCmd.setMainId(addCmd.getId());
        variableAddCmd.setWorkflowId(workflowDo.getId());
        variableAddCmd.setFormData(JSON.toJSONString(addCmd));
        variableAddCmd.setProcessData(JSON.toJSONString(dto));
        variableAddCmd.setMemo("重新发起");
        iLmcWorkflowVariableService.add(variableAddCmd);
...
//        timelineAddCmd.setId(0L);
    ...
    @Override
    public Response draftProcess(ProjectRecordsAddCmd addCmd) {
        Boolean b = FunctionalUtil.verifyAndParseApproveUser(addCmd.getApproveUser());
        if (!b) {
            return Response.buildFailure("505","请指定审批人！");
        }
...
//        String fdId = projectRecordsService.addThenReturnId(addCmd);
        ...
        projectRecordsDo.setUpdatedTime(LocalDateTime.now());
        boolean bool = projectRecordsRepository.updateById(projectRecordsDo);
        if (!bool) log.error("发起流程:更新新增项目登记时失败!项目登记:{}", projectRecordsDo);
        Long fdId = projectRecordsDo.getId();
...
//        timelineAddCmd.setId(0L);
    ...
    public void handlePass(ProcessCall processCall) {
        ...
        //项目等级审批通过，添加通过日期，定时任务需要
        ProjectRecordsUpdateCmd updateCmd = new ProjectRecordsUpdateCmd();
        updateCmd.setId(Long.valueOf(processCall.getBusinessKey()));
        updateCmd.setSuccessTime(LocalDate.now());//设置当前日期
