...
public class SubmitAddProjectSigningServiceImpl implements com.uwdp.workflow.service.iservice.IProjectSigningService {


    @Value("${hpcc-mim-server.app-id}")
    public String appId; /*应用ID*/

    private static final String flowCode = BizCode.ADD_PROJECT_SIGNING.getValue(); /*流程编码*/
    ...
    @Override
    public Response add(ProjectSigningAddCmd addCmd) {
        ...
        Long id = projectSigningService.addThenReturnId(addCmd);
        ...
        dto.setTitle("【市场管理】【"+addCmd.getProjectName()+"】项目签约审批单");
...
//            lmcWorkflowDo.setId();
        ...
        lmcWorkflowDo.setTitle("项目签约新增-"+addCmd.getContracName());
    ...
    public Response reStartProcess(ProjectSigningAddCmd addCmd) {
        ...
        LmcWorkflowDo workflowDo = Optional.ofNullable(
                ...
                () -> new BizRuntimeException(
                        "根据新增项目签约的Id:{" + addCmd.getId() + "}找不到对应流程实例",new Exception())
        );
        ProjectSigningDo projectSigningDo = projectSigningAssembler.toDO(addCmd);
        projectSigningDo.setUpdatedTime(LocalDateTime.now());
        boolean bool = projectSigningRepository.updateById(projectSigningDo);
        if (!bool)log.error("重新发起流程:更新新增项目签约时失败!项目签约:{}",projectSigningDo);
        Map<String, Object> variables = BeanUtil.beanToMap(addCmd);
...
//        dto.setBusinessPositions();     // 业务岗
        ...
        log.info("==项目签约新增流程驳回后重新发起 响应:{}",response);
...
//        String fdId = projectSigningService.addThenReturnId(addCmd);
        ...
        projectSigningDo.setUpdatedTime(LocalDateTime.now());
        boolean bool = projectSigningRepository.updateById(projectSigningDo);
        if (!bool)log.error("发起流程:更新新增项目签约时失败!项目签约:{}",projectSigningDo);
        Long fdId=projectSigningDo.getId();
...
//        timelineAddCmd.setId(0L);
    ...
    public void handlePass(ProcessCall processCall) {
        ...
        projectRecordsQuery.setProjectPhase("5");
