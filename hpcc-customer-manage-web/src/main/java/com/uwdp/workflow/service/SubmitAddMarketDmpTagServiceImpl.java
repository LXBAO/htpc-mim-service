...
import com.uwdp.workflow.service.iservice.IMarketDmpTagService;
...
public class SubmitAddMarketDmpTagServiceImpl implements IMarketDmpTagService {

    @Value("${hpcc-mim-server.app-id}")
    public String appId; /*应用ID*/

    private static final String flowCode = BizCode.ADD_MARKET_DMP_TAG.getValue(); /*流程编码*/

    private final com.uwdp.module.api.service.IMarketDmpTagService marketDmpTagService;
    ...
    @Override
    public Response add(MarketDmpTagAddCmd addCmd) {
...
//        addCmd.setPersonName(personDo.getPersonName());
        ...
        Long id = marketDmpTagService.addThenReturnId(addCmd);
        ...
        dto.setTitle("【市场管理】【"+addCmd.getTitle()+"】指标审批单");
...
//            lmcWorkflowDo.setId();
        ...
        lmcWorkflowDo.setTitle("指标新增-"+addCmd.getTitle());
...
//        MarketDmpTagDo marketDmpTag = marketDmpTagAssembler.toDO(addCmd);
//        marketDmpTag.setUpdatedTime(LocalDateTime.now());
//        boolean bool = marketDmpTagRepository.updateById(marketDmpTag);
//        if (!bool)log.error("重新发起流程:更新新增项目投标时失败!项目投标:{}",marketDmpTag);
        // 数据权限 添加创建人字段(保存创建人personCode)
        MdmPersonDo personDo = mdmPersonUtils
                .checkIfPersonCodeExist(addCmd.getCreatedBy());
        String groupFullCode = mdmPersonUtils.checkIfOrganizationExist(
        ...
        // 数据权限 添加部门全路径字段(保存创建人所属部门全路径)
        addCmd.setGroupFullCode(groupFullCode);
        addCmd.setUpdatedTime(LocalDateTime.now());
        marketDmpTagService.update(addCmd);
...
//        MarketDmpTagDo marketDmpTagDo = marketDmpTagAssembler.toDO(addCmd);
//        marketDmpTagDo.setUpdatedTime(LocalDateTime.now());
//        boolean bool = marketDmpTagRepository.updateById(marketDmpTagDo);
//        if (!bool)log.error("发起流程:更新新增项目投标时失败!项目投标:{}",marketDmpTagDo);
        Long fdId=addCmd.getId();
