 ...
 */
@RestController
@RequestMapping("/hpcc-customer-manage/v1_0/module/platform-ledger-v1-0-0")
@Api(tags = "平台台账")
...
public class PlatformLedgerController {
    ...
    public PageResponse<PlatformLedgerDto> page(PlatformLedgerQuery paramQuery, HttpServletRequest request) {
        try {
            Map<String, Object> map = MapUtils.flatBuilder(request.getParameterMap())
                    ...
                    .field(PlatformLedgerDto::getCreatedName).op(Contain.class)
    ...
    @ApiOperation(value = "详情接口", notes="platformLedger")
    public SingleResponse<PlatformLedgerDto> getPassword(@ApiParam(value = "主键", required = true) @RequestParam(name = "fdId") Long fdId) {
        try {
            return SingleResponse.of(platformLedgerService.getPassword(fdId));
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    }
    ...
    @PostMapping("/add")
    @ApiOperation(value = "新增接口", notes="platformLedger")
    @OperationLog(type = OperateTypeEnum.ADD)
    public Response add(@RequestBody @Validated PlatformLedgerAddCmd addCmd) {
        try {
            platformLedgerService.add(addCmd);
    ...
    @PutMapping("/update")
    @ApiOperation(value = "编辑接口", notes="platformLedger")
    @OperationLog(type = OperateTypeEnum.UPDATE)
    public Response update(@RequestBody @Validated PlatformLedgerUpdateCmd updateCmd) {
        try {
            platformLedgerService.update(updateCmd);
            return Response.buildSuccess();
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    ...
    @ApiOperation(value = "删除接口", notes="platformLedger")
    ...
    public Response delete(@ApiParam(value = "主键", required = true) @RequestParam(name = "fdId") Long fdId) {
        try {
            platformLedgerService.delete(fdId);
            return Response.buildSuccess();
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    ...
    @ApiOperation(value = "下载Excel模板", notes="platformLedger")
    @OperationLog(type = OperateTypeEnum.DOWNLOAD)
    public void excelTemplate(HttpServletResponse response) {
        try {
            platformLedgerService.excelTemplate(response);
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    }
    ...
    @ApiOperation(value = "导出Excel数据, 最多3000条", notes="platformLedger")
    @OperationLog(type = OperateTypeEnum.EXPORT)
    public void excelExport(HttpServletRequest request, HttpServletResponse response) {
    Map<String, Object> map = MapUtils.flat(request.getParameterMap());
        try {
            platformLedgerService.excelExport(map, response);
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    }
    ...
    @ApiOperation(value = "解析上传的Excel数据", notes="platformLedger")
    public SingleResponse<ExcelParseDTO<PlatformLedgerExcelImport>> excelParse(@RequestParam(value = "file", required = false) MultipartFile file) {
        try {
            return SingleResponse.of(platformLedgerService.excelParse(file));
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    }
    ...
    @ApiOperation(value = "导入处理后的Excel数据", notes="platformLedger")
    @OperationLog(type = OperateTypeEnum.IMPORT)
    public Response excelImport(@RequestBody @Valid List<PlatformLedgerExcelImport> list) {
        try {
            platformLedgerService.excelImport(list);
            return Response.buildSuccess();
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    ...
    @ApiOperation(value = "导出错误的数据", notes="platformLedger")
    @OperationLog(type = OperateTypeEnum.DOWNLOAD)
    public void excelErrorDownload(@RequestBody List<PlatformLedgerExcelExport> list, HttpServletResponse response) {
        try {
            platformLedgerService.excelErrorDownload(list, response);
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    }
