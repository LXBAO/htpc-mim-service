 ...
 */
@RestController
@RequestMapping("/hpcc-customer-manage/v1_0/module/relevant-contact-v1-0-0")
@Api(tags = "客户相关联系人")
@RequiredArgsConstructor
@OperationModule(type = ModuleTypeEnum.APP, business = "客户相关联系人")
@Validated
public class RelevantContactController {
    ...
    @PostMapping("/add")
    @ApiOperation(value = "新增接口", notes="relevantContact")
    @OperationLog(type = OperateTypeEnum.ADD)
    public Response add(@RequestBody @Validated RelevantContactAddCmd addCmd) {
        try {
            relevantContactService.add(addCmd);
    ...
    @PutMapping("/update")
    @ApiOperation(value = "编辑接口", notes="relevantContact")
    @OperationLog(type = OperateTypeEnum.UPDATE)
    public Response update(@RequestBody @Validated RelevantContactUpdateCmd updateCmd) {
        try {
            relevantContactService.update(updateCmd);
            return Response.buildSuccess();
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    ...
    @ApiOperation(value = "删除接口", notes="relevantContact")
    ...
    public Response delete(@ApiParam(value = "主键", required = true) @RequestParam(name = "id") Long id) {
        try {
            relevantContactService.delete(id);
            return Response.buildSuccess();
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    ...
    @ApiOperation(value = "下载Excel模板", notes="relevantContact")
    @OperationLog(type = OperateTypeEnum.DOWNLOAD)
    public void excelTemplate(HttpServletResponse response) {
        try {
            relevantContactService.excelTemplate(response);
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    }
    ...
    @ApiOperation(value = "导出Excel数据, 最多3000条", notes="relevantContact")
    @OperationLog(type = OperateTypeEnum.EXPORT)
    public void excelExport(HttpServletRequest request, HttpServletResponse response) {
    Map<String, Object> map = MapUtils.flat(request.getParameterMap());
        try {
            relevantContactService.excelExport(map, response);
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    }
    ...
    @ApiOperation(value = "解析上传的Excel数据", notes="relevantContact")
    public SingleResponse<ExcelParseDTO<RelevantContactExcelImport>> excelParse(@RequestParam(value = "file", required = false) MultipartFile file) {
        try {
            return SingleResponse.of(relevantContactService.excelParse(file));
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    }
    ...
    @ApiOperation(value = "导入处理后的Excel数据", notes="relevantContact")
    @OperationLog(type = OperateTypeEnum.IMPORT)
    public Response excelImport(@RequestBody @Valid List<RelevantContactExcelImport> list) {
        try {
            relevantContactService.excelImport(list);
            return Response.buildSuccess();
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    ...
    @ApiOperation(value = "导出错误的数据", notes="relevantContact")
    @OperationLog(type = OperateTypeEnum.DOWNLOAD)
    public void excelErrorDownload(@RequestBody List<RelevantContactExcelExport> list, HttpServletResponse response) {
        try {
            relevantContactService.excelErrorDownload(list, response);
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    }
