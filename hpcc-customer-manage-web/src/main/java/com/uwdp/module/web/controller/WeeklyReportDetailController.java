 ...
 */
@RestController
@RequestMapping("/hpcc-customer-manage/v1_0/module/weekly-report-detail-v1-0")
@Api(tags = "周报明细表")
@RequiredArgsConstructor
@OperationModule(type = ModuleTypeEnum.APP, business = "周报明细表")
@Validated
public class WeeklyReportDetailController {
    ...
    @PostMapping("/add")
    @ApiOperation(value = "新增接口", notes="weekly_Report_Detail")
    @OperationLog(type = OperateTypeEnum.ADD)
    public Response add(@RequestBody @Validated WeeklyReportDetailAddCmd addCmd) {
        try {
            weeklyReportDetailService.add(addCmd);
    ...
    @PutMapping("/update")
    @ApiOperation(value = "编辑接口", notes="weekly_Report_Detail")
    @OperationLog(type = OperateTypeEnum.UPDATE)
    public Response update(@RequestBody @Validated WeeklyReportDetailUpdateCmd updateCmd) {
        try {
            weeklyReportDetailService.update(updateCmd);
            return Response.buildSuccess();
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    ...
    @ApiOperation(value = "删除接口", notes="weekly_Report_Detail")
    ...
    public Response delete(@ApiParam(value = "主键", required = true) @RequestParam(name = "id") Long id) {
        try {
            weeklyReportDetailService.delete(id);
            return Response.buildSuccess();
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    ...
    @ApiOperation(value = "批量删除接口, ids以逗号分隔多个id", notes="weekly_Report_Detail")
    @OperationLog(type = OperateTypeEnum.DELETE)
    public Response batchDelete(@ApiParam(value = "主键列表, 多个主键以逗号分割", required = true) @RequestParam(name = "ids") String ids) {
        try {
            weeklyReportDetailService.batchDelete(ids);
            return Response.buildSuccess();
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    ...
    @ApiOperation(value = "下载Excel模板", notes="weekly_Report_Detail")
    @OperationLog(type = OperateTypeEnum.DOWNLOAD)
    public void excelTemplate(HttpServletResponse response) {
        try {
            weeklyReportDetailService.excelTemplate(response);
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    }
    ...
    @ApiOperation(value = "导出Excel数据, 最多3000条", notes="weekly_Report_Detail")
    @OperationLog(type = OperateTypeEnum.EXPORT)
    public void excelExport(HttpServletRequest request, HttpServletResponse response) {
    Map<String, Object> map = MapUtils.flat(request.getParameterMap());
        try {
            weeklyReportDetailService.excelExport(map, response);
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    }
    ...
    @ApiOperation(value = "解析上传的Excel数据", notes="weekly_Report_Detail")
    public SingleResponse<ExcelParseDTO<WeeklyReportDetailExcelImport>> excelParse(@RequestParam(value = "file", required = false) MultipartFile file) {
        try {
            return SingleResponse.of(weeklyReportDetailService.excelParse(file));
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    }
    ...
    @ApiOperation(value = "导入处理后的Excel数据", notes="weekly_Report_Detail")
    @OperationLog(type = OperateTypeEnum.IMPORT)
    public Response excelImport(@RequestBody @Valid List<WeeklyReportDetailExcelImport> list) {
        try {
            weeklyReportDetailService.excelImport(list);
            return Response.buildSuccess();
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    ...
    @ApiOperation(value = "导出错误的数据", notes="weekly_Report_Detail")
    @OperationLog(type = OperateTypeEnum.DOWNLOAD)
    public void excelErrorDownload(@RequestBody List<WeeklyReportDetailExcelExport> list, HttpServletResponse response) {
        try {
            weeklyReportDetailService.excelErrorDownload(list, response);
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    }
