 ...
 */
@RestController
@RequestMapping("/hpcc-customer-manage/v1_0/module/mdm-organization-v1-0-0")
@Api(tags = "主数据-组织")
@RequiredArgsConstructor
@OperationModule(type = ModuleTypeEnum.APP, business = "主数据-组织")
@Validated
public class MdmOrganizationController {
    ...
    @PostMapping("/add")
    @ApiOperation(value = "新增接口", notes="mdmOrganization")
    @OperationLog(type = OperateTypeEnum.ADD)
    public Response add(@RequestBody @Validated MdmOrganizationAddCmd addCmd) {
        try {
            mdmOrganizationService.add(addCmd);
    ...
    @PutMapping("/update")
    @ApiOperation(value = "编辑接口", notes="mdmOrganization")
    @OperationLog(type = OperateTypeEnum.UPDATE)
    public Response update(@RequestBody @Validated MdmOrganizationUpdateCmd updateCmd) {
        try {
            mdmOrganizationService.update(updateCmd);
            return Response.buildSuccess();
        } catch (Exception e) {
            throw new BizRuntimeException(e);
            }
    ...
    @ApiOperation(value = "删除接口", notes="mdmOrganization")
    ...
    public Response delete(@ApiParam(value = "主键", required = true) @RequestParam(name = "id") Long id) {
        try {
            mdmOrganizationService.delete(id);
            return Response.buildSuccess();
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    ...
    @ApiOperation(value = "下载Excel模板", notes="mdmOrganization")
    @OperationLog(type = OperateTypeEnum.DOWNLOAD)
    public void excelTemplate(HttpServletResponse response) {
        try {
            mdmOrganizationService.excelTemplate(response);
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    }
    ...
    @ApiOperation(value = "导出Excel数据, 最多3000条", notes="mdmOrganization")
    @OperationLog(type = OperateTypeEnum.EXPORT)
    public void excelExport(HttpServletRequest request, HttpServletResponse response) {
    Map<String, Object> map = MapUtils.flat(request.getParameterMap());
        try {
            mdmOrganizationService.excelExport(map, response);
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    }
    ...
    @ApiOperation(value = "解析上传的Excel数据", notes="mdmOrganization")
    public SingleResponse<ExcelParseDTO<MdmOrganizationExcelImport>> excelParse(@RequestParam(value = "file", required = false) MultipartFile file) {
        try {
            return SingleResponse.of(mdmOrganizationService.excelParse(file));
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    }
    ...
    @ApiOperation(value = "导入处理后的Excel数据", notes="mdmOrganization")
    @OperationLog(type = OperateTypeEnum.IMPORT)
    public Response excelImport(@RequestBody @Valid List<MdmOrganizationExcelImport> list) {
        try {
            mdmOrganizationService.excelImport(list);
            return Response.buildSuccess();
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    ...
    @ApiOperation(value = "导出错误的数据", notes="mdmOrganization")
    @OperationLog(type = OperateTypeEnum.DOWNLOAD)
    public void excelErrorDownload(@RequestBody List<MdmOrganizationExcelExport> list, HttpServletResponse response) {
        try {
            mdmOrganizationService.excelErrorDownload(list, response);
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    }
