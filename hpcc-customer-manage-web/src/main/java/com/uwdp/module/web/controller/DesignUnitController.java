 ...
 */
@RestController
@RequestMapping("/hpcc-customer-manage/v1_0/module/design-unit-v1")
@Api(tags = "设计单位")
...
public class DesignUnitController {
    ...
    @PostMapping("/add")
    @ApiOperation(value = "新增接口", notes="designUnit")
    @OperationLog(type = OperateTypeEnum.ADD)
    public Response add(@RequestBody @Validated DesignUnitAddCmd addCmd) {
        try {
            designUnitService.add(addCmd);
    ...
    @PutMapping("/update")
    @ApiOperation(value = "编辑接口", notes="designUnit")
    @OperationLog(type = OperateTypeEnum.UPDATE)
    public Response update(@RequestBody @Validated DesignUnitUpdateCmd updateCmd) {
        try {
            designUnitService.update(updateCmd);
            return Response.buildSuccess();
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    ...
    @ApiOperation(value = "删除接口", notes="designUnit")
    ...
    @ApiOperation(value = "下载Excel模板", notes="designUnit")
    @OperationLog(type = OperateTypeEnum.DOWNLOAD)
    public void excelTemplate(HttpServletResponse response) {
        try {
            designUnitService.excelTemplate(response);
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    }
    ...
    @ApiOperation(value = "导出Excel数据, 最多3000条", notes="designUnit")
    @OperationLog(type = OperateTypeEnum.EXPORT)
    public void excelExport(HttpServletRequest request, HttpServletResponse response) {
    Map<String, Object> map = MapUtils.flat(request.getParameterMap());
        try {
            designUnitService.excelExport(map, response);
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    }
    ...
    @ApiOperation(value = "解析上传的Excel数据", notes="designUnit")
    public SingleResponse<ExcelParseDTO<DesignUnitExcelImport>> excelParse(@RequestParam(value = "file", required = false) MultipartFile file) {
        try {
            return SingleResponse.of(designUnitService.excelParse(file));
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    }
    ...
    @ApiOperation(value = "导入处理后的Excel数据", notes="designUnit")
    @OperationLog(type = OperateTypeEnum.IMPORT)
    public Response excelImport(@RequestBody @Valid List<DesignUnitExcelImport> list) {
        try {
            designUnitService.excelImport(list);
            return Response.buildSuccess();
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    ...
    @ApiOperation(value = "导出错误的数据", notes="designUnit")
    @OperationLog(type = OperateTypeEnum.DOWNLOAD)
    public void excelErrorDownload(@RequestBody List<DesignUnitExcelExport> list, HttpServletResponse response) {
        try {
            designUnitService.excelErrorDownload(list, response);
        } catch (Exception e) {
            throw new BizRuntimeException(e);
        }
    }
